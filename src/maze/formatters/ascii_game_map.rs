...
/// A AsciiGameMap formatter for a generated maze
///
...
impl AsciiGameMap {
    /// Returns a new instance of an [AsciiGameMap] formatter with a default settings
    pub fn new() -> AsciiGameMap {
        AsciiGameMap {
            span: 2,
            wall: '#',
            passage: '.',
...
impl Formatter<StringWrapper> for AsciiGameMap {
...
mod tests {
    ...
    fn new_call_default_params() {
        let formatter = AsciiGameMap::new();
        assert_eq!(2, formatter.span);
    }

    #[test]
    fn span_change() {
        let formatter = AsciiGameMap::new().span(10);
