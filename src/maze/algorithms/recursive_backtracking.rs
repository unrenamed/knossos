...
///
/// This algorithm quite effectively creates narrow passages with multiple dead-ends which makes it easy to
/// get lost, thus eventually making it hard to solve the maze.
///
/// In most cases, this algorithm is fast. However, due to its recursive nature, it requires stack space
/// proportional to the longest acyclic path, which is, in the worst case, the entire maze. So for exceptionally
/// large mazes this algorithm can be fairly inefficient.
...
/// 3. If all adjacent cells have been visited, backs up to the last cell that has uncarved walls and repeats
